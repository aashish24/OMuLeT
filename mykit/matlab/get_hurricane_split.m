...
%   Detailed explanation goes here
    ...
    t=ones(length(opts.time_year),1)*-1;
    for h=1:num
        for k=1:length(opts.time_year)
            if start_time(h)>=opts.time_year(k) && t(k)==-1
                t(k)=h;
            end
        end
    end
    for k=1:length(opts.time_year)+1
        if k==1
            year_idx{k}=idx(1:t(k)-1);
        elseif k==length(opts.time_year)+1
            year_idx{k}=idx(t(k-1):end);    
        else
            year_idx{k}=idx(t(k-1):t(k)-1);
        end
    end
    t1=-1;
    t2=-1;
...
%     train_idx(randperm(num,ceil(num*opts.ratio)))=true;
    ...
    length(vali_idx)
    length(test_idx)
    save(sprintf('%s/train_test_idx.mat',filename_all.data_dir),'train_idx','test_idx','vali_idx','year_idx');
