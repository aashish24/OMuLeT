...
%         geoshow('landareas.shp','FaceColor',[0.9294    0.7412    0.3961]);
        ...
        h{num_model+1}=geoshow(X(2,idx),X(1,idx),'displaytype','line','color','r');
        Y=fc.Y(:,t:t+8);
        h{num_model+2}=geoshow(Y(2,:),Y(1,:),'displaytype','line','color','g');
...
%         yc=floor(mean(ax.YLim));
        ...
        xlabel('Longitude');
        ylabel('Latitude');
        legend([h{m+2} h{m+1} h{1}],{'Best track', 'Ens_{median}', 'AP ens members'})
        F(t) = getframe(gcf);
        for m=1:num_model+2
            delete (h{m});
        end
