...
%         ht_a = ht_b(idx,:);
        ...
        forecast_tensor(h).X = ones(2,n,opts.beta+1,t_num)*-1000;
        forecast_tensor(h).X_flag = false(n,opts.beta,t_num);
        forecast_tensor(h).nhc = ones(2,opts.beta+1,t_num)*-1000;
        forecast_tensor(h).Y = ones(2,t_num+opts.beta)*-1000;
        forecast_tensor(h).label = zeros(opts.beta,t_num);
        forecast_tensor(h).predict = ones(2,opts.beta,t_num)*-1000; 
...
%         forecast_tensor(h).forecast = ones(t_num,opts.beta,n,2)*-1000;
        ...
        for p = 1:size(bt,1)
            ...
            if abs(floor(t)-t)<0.01 && t>0 && t<t_num+opts.beta
                forecast_tensor(h).Y(:,t)=bt(p,2:3);
            end
        end
        ...
        for p = 1:size(fc_b,1)
            ...
            if abs(floor(t)-t)<0.01 && abs(floor(tau)-tau)<0.01
                forecast_tensor(h).X(:,m,tau,t)=fc_b(p,4:5);
            end
        end
        ...
        for p = 1:size(fc_b_nhc,1)
            ...
            if t>0 && abs(floor(t)-t)<0.01 && abs(floor(tau)-tau)<0.01
                forecast_tensor(h).nhc(:,tau,t)=fc_b_nhc(p,4:5);
            end
        end
...
%         end
        ...
        % Interpolation for X
        for t = 1:size(forecast_tensor(h).X,4)
            for m = 1:size(forecast_tensor(h).X,2)
        ...
        for t = 1:size(forecast_tensor(h).X,4)
            for tau = 1:size(forecast_tensor(h).X,3)
                if forecast_tensor(h).Y(1,t+tau)~=-1000 && forecast_tensor(h).Y(2,t+tau)~=-1000
                    ...
                    forecast_tensor(h).label(tau,t)=0;
                end
                forecast_tensor(h).X_flag(:,tau,t)=sum(forecast_tensor(h).X(:,:,tau,t)==-1000)==0;
            end
        end
        
        % get maxinum complete part of X
        forecast_bool=reshape(forecast_tensor(h).X_flag,[m,size(forecast_tensor(h).X_flag,3)*size(forecast_tensor(h).X_flag,2)]);
%         idx=false(m,1);
%         count=0;
%         while(true)
%             m_add_max=0;
...
%             end
%         end
        idx=sum(forecast_bool,2)/size(forecast_bool,2);
        idx=idx>=opts.models_percent;
        forecast_tensor(h).X=forecast_tensor(h).X(:,idx,:,:);
        forecast_tensor(h).X_flag=forecast_tensor(h).X_flag(idx,:,:);
