function get_stat_one_hurricane(opts)
            ...
            for t=1:size(forecast_tensor(h).X,4)
                for tau=1:opts.beta
                    if forecast_tensor(h).label(tau,t)>0
                        ...
                        predict_mean=mean(X,1);
                        predict_median=median(X,1);
...
%         error(ii) = sum(error_online([2 4 6 8]));
    ...
    hold on;
    tau=8
    h1=plot(2012:2017,error_mean(:,tau),'k-');
%     plot(error_median(:,tau*2),'g-');
    h2=plot(2012:2017,error_nhc(:,tau),'b-');
    h3=plot(2012:2017,error_online(:,tau),'r-');
    lg=legend([h3,h2,h1],{'OMuLT' 'NHC' 'Ensenble Mean'},'FontSize',12);
    xticks(2012:2017);
    xlabel('Year');
