function opts = convert_nhc_fc_txt(opts)
...
%   Detailed explanation goes here
    ...
    listing = dir(sprintf('%s/NHC/adv_track_txt',filename_all.base_dir));
    num = numel(listing)-2;
    ...
    for p=1:num
        ...
        if ~isempty(hurricane_id)
            ...
            expr = 'FORECAST/ADVISORY NUMBER.+?\n.+?\n(?<date>.+?)\n.+?REPEAT...CENTER LOCATED NEAR.+?\nAT (?<time>.+?) CENTER WAS LOCATED NEAR (?<lat>.+?)\s+(?<lon>.+?)\n(?<data>.+?)NNNN';
            obj = regexp(txt,expr,'names');
            
            date=split(obj.date,' ');
            n=length(date);
            date=sprintf('%s-%s-%s',date{n-2},date{n-1},date{n});
            date=datenum(date,'m-dd-yyyy')
