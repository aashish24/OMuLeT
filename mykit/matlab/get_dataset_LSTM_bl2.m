...
%   Detailed explanation goes here
    ...
    for p=1:length(train_idx)
        ...
        t1 = min(fc2(:,1));
        t2 = t1+0.25*opts.window_size;
        s=opts.window_size*(opts.forecast_time/6+1);
        while t2 <= max(fc2(:,1))
            idx = fc2(:,1)>=t1 & fc2(:,1)<t2;
            if sum(idx)==s
                num=num+1;
                train.X=[train.X; reshape(fc2(idx,[4:5]),1,s*2)];
                train.y=[train.y; reshape(ht2(idx,[3:4]),1,s*2)];
            end
            t1=t1+0.25;
            t2=t2+0.25;
        end
    ...
    for p=1:length(test_idx)
        ...
        while t2 <= max(fc2(:,1))
            ...
            if sum(idx)==s
                num=num+1;
                test.X=[test.X; reshape(fc2(idx,[4:5]),1,s*2)];
                test.y=[test.y; reshape(ht2(idx,[4:5]),1,s*2)];
            end
            t1=t1+0.25;
            t2=t2+0.25;
        end
    ...
    csvwrite(sprintf('%s/%s_train_X.csv',filename_all.data_dir,filename),train.X);
    csvwrite(sprintf('%s/%s_train_y.csv',filename_all.data_dir,filename),train.y);
    csvwrite(sprintf('%s/%s_test_X.csv',filename_all.data_dir,filename),test.X);
    csvwrite(sprintf('%s/%s_test_y.csv',filename_all.data_dir,filename),test.y);
